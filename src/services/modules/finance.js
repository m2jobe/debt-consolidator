...
;(function (root) {
  ...
  lib.format = function (num, settings, override) {
    ...
    // set precision
    if (settings.precision != null) num = num.toFixed(settings.precision)
  ...
  lib.calculateMonths = function (finAmount, finInterest, finPayment) {
    ...
    if (finInterest == 0) {
      ...
      result = Math.round(
        (((-1 / 12) *
          Math.log(1 - (finAmount / finPayment) * (finInterest / 100 / 12))) /
  ...
  lib.calculateEarlyPayoff = function (
    finAmount,
    finInterest,
    finMonths,
    finRemainingMonths,
    ...
    var principle1 = finAmount,
      interest1 = 0,
      principle2 = finAmount,
      interest2 = 0
    var ep
